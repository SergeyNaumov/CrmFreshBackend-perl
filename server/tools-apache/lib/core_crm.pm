...
sub login{
  ...
  my ($manager_id,$auth_key);
  if($ENV{HTTP_COOKIE}=~m/auth_user_id=(\d+)/){
    $manager_id=$1;
  }
  if($ENV{HTTP_COOKIE}=~m/auth_key=([a-zA-Z0-9]+)/){
    $auth_key=$1;
  }
  my $dbh=$arg{connect};
  my $sth=$dbh->prepare("select * from session where auth_id=? and session_key=?");
  $sth->execute($manager_id,$auth_key);
  if(my $m=$sth->fetchrow_hashref){
    return $m->{auth_id}
  }
  return undef;
  #&::pre([$manager_id,$auth_key]);
}
